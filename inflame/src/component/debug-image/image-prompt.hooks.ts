 ...
 */
export const usePostSimpleImagePrompt = (props: { socket: GenericSocket; promptName?: string }) => {
    const { socket, promptName = 'usePostSimpleImagePrompt' } = props

    const [postPrompt, requestMeta] = comfyApi.usePostPrompt()
    const dispatch = useAppDispatch()
    const prompt = useSelector((state: AppState) =>
    ...
    useEffect(() => {
        ...
        if (loadImage.state.image) {
            loadImage.inputs.image = {
                kind: 'string',
                value: loadImage.state.image[0][1] ?? '',
            }
        } else {
