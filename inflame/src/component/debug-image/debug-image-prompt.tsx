...
const DebugImagePrompt = () => {
    ...
    const socket = useSelector(
        (state: AppState): GenericSocket =>
            socketStateSelectors.selectSocketById(state, COMFYUI_SOCKET)
    )
    ...
    return (
        <Box p={4}>
            ...
            <Box pb={4}>
                <Fileuploader fileTypes={['image/jpeg', 'image/png']} />
            </Box>
            <Box>
            ...
            <Box>
                <AbstractNodeView
                    abstractNode={debugPrompt?.workflow.nodes.at(0)}
                ></AbstractNodeView>
            </Box>
        </Box>
    )
