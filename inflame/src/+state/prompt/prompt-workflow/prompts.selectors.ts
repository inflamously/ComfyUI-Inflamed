...
const selectPromptByName = createSelector(
    [selectPromptsStateItems, (_: AppState, name: string) => name],
...
const selectPromptByRemoteId = createSelector(
    [selectPromptsStateItems, (_, remoteId: string) => remoteId],
    (state, remoteId: string) =>
        promptsEntityAdapterSelectors.selectAll(state).find((i) => i.remoteId === remoteId)
)

const selectPrompts = createSelector(selectPromptsStateItems, (state: PromptsEntityAdapterType) =>
    promptsEntityAdapterSelectors.selectAll(state)
)

const selectPromptNames = createSelector(selectPrompts, (prompts: Prompt[] | undefined) =>
    prompts ? prompts.map((p) => p.name) : []
)

const selectPromptsByNewest = createSelector(selectPrompts, (prompts: Prompt[] | undefined) =>
    prompts ? prompts[prompts.length - 1] : undefined
)
